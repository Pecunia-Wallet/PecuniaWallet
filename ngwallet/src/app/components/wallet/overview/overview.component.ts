...
import {forkJoin, map, skip, take} from "rxjs";
import {HttpClient, HttpContext} from "@angular/common/http";
import {binance, CSRF, sleep} from "../../../app.config";
...
export class OverviewComponent implements OnInit {
    ...
    private getPortfolioValueChange(coins: Coin[], balances: BigNumber[], fiat: FiatCurrency) {
        const queries = coins.flatMap(coin => {
            let fiatSymbol = fiat.shortName;
            if (fiatSymbol === "USD") fiatSymbol = "USDT";
            const symbol = (coin.shortName + fiatSymbol).toUpperCase();
            return {coin, querySymbol: symbol};
        });

        const symbols = queries.map(q => `"${q.querySymbol}"`).join(",");
        return this.http.get<Array<{
            symbol: string,
            openPrice: string | BigNumber,
            lastPrice: string | BigNumber,
            coin: Coin
        }>>(encodeURI(`${binance}/v3/ticker/24hr?type=MINI&symbols=[${symbols}]`), {
            ...
            changes.map(change => {
                ...
                const coin = queries.find(q => q.querySymbol == change.symbol)?.coin;
        ...
        forkJoin(this.coins.map(c => this.wallet.getBalance(c).pipe(take(1))))
            .subscribe(balances => {
                forkJoin([...balances.map(((balance, i) =>
                    ...
                    this.getPortfolioValueChange(this.coins, balances, this.accountCurrency)
    ...
    ngOnInit() {
        ...
        forkJoin([
            ...
            setTimeout(() => {
                ...
                this.coins.forEach(coin => this.wallet.onBalanceChange(coin).pipe(skip(1))
                    .subscribe(_ => {
