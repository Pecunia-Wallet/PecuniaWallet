        ...
        try {
            ...
            const queries = coins.flatMap(coin =>
                fiats.map(fiat => {
                    ...
                    return {coin, fiat, querySymbol: symbol};
                })
            );

            const symbols = queries.map(q => `"${q.querySymbol}"`).join(",");
            const bRates = await firstValueFrom(
                this.http.get<Array<{ symbol: string; price: string }>>(
                    encodeURI(`${binance}/v3/ticker/price?symbols=[${symbols}]`), {
                        context: new HttpContext().set(CSRF, false)
                    }));

            // @ts-ignore as soon as we filter results
            const rates: Rate[] = bRates.map(bRate => {
                const query = queries.find(q => q.querySymbol === bRate.symbol);
                if (!query) return null;
                return new Rate(query.coin, query.fiat, new BigNumber(bRate.price));
            }).filter(rate => !!rate);
